...
def get_oracle_table_names(connection_string):
    ...
    except oracledb.DatabaseError as e:  # 异常类型更新为 oracledb.DatabaseError
        print(f"数据库连接失败：{e}")
        return []
...
def get_ora_table_rows(connection_string, user, Table_O):
    try:
        ...
        query = """
            ...
            ORDER BY column_id
        """
        cursor.execute(query, {"owner": user, "table_name": Table_O})
        columns = cursor.fetchall()

        list_column_name = [column[0] for column in columns]  # 简化字段名提取
...
#             FROM all_tab_columns
#             WHERE owner = UPPER('{user}') AND table_name = UPPER('{Table_O}')
#             ORDER BY column_id"""
#
#         cursor.execute(query)
#         columns = cursor.fetchall()
...
def supply_deficiency(table, list):
...
def this_main_map_name(connection_string,user, table_D, table_O, POST):
    ...
    for row in rows:
        ...
        if str == '':
            ...
            list.append(POST.get(row, ''))
    str_sup = supply_deficiency(table_D, list)
