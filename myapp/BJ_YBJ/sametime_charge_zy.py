...
def generate_diag_conditions(input_string):
    ...
    if '、' in input_string:
        ...
        items = [input_string]
    # 为每个项创建LIKE条件，并用'or'连接
    conditions = [f"入院诊断名称||','|| 出院诊断名称 not like '%{item}%'" for item in items]
    # 使用'or'连接所有的条件
    sql_conditions = 'and '+'\nand '.join(conditions)
    if (input_string == ''):
        return ''
...
def this_main_zy_sametime(p1,p2,diag):
  ...
  WHERE
    """
    part2 = """ GROUP BY  结算单据号, 项目使用日期::DATE HAVING SUM(数量)>0
  INTERSECT
  SELECT
    结算单据号,
    项目使用日期::DATE 项目使用日期
  ...
  WHERE
    """
    part3 = """ GROUP BY  结算单据号, 项目使用日期::DATE HAVING SUM(数量)>0
)
SELECT
  A.病案号,
  A.结算单据号,
  ...
  JOIN tab1 C ON B.结算单据号 = C.结算单据号 AND C.项目使用日期 = B.项目使用日期::DATE
WHERE"""
